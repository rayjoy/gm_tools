...
- ZUC：支持祖冲之序列密码算法。
    ...
    - **ZUC-256 (新初始化)**：支持 32/64/128 位完整性校验值 (MAC) 生成。
...
3) SM4 子系统
- 使用 `sm4` crate，提供 ECB, CBC 与 GCM 模式。
- ECB/CBC 模式：
    - 支持可选的 PKCS#7 填充（默认启用）。
    - 若启用填充：加密时自动填充，解密时自动去填充。
    - 若禁用填充：输入必须为 16 字节整数倍，直接进行块加密/解密。
- GCM 模式：支持认证加密，需提供 Nonce 和 AAD。
